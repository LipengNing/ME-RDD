function ShowDDF2_exvivo(p,D,r,D_max,r_max,trim,pmax,sc)
if(~exist('D_max','var'))
    D_max=2;
end
...
%set(h,'linewidth',.1);
%mesh(r,D,pp);
set(gca,'CameraPosition',[16.375 1123.8 1.08]);%,'CameraViewAngle',10);
...
hold on;plot3(D(3:(end-trim)),zeros(1,Nd-2-trim),pD*sc,'linewidth',3);%3 10
plot3(zeros(1,Nr-2-trim),r(3:(end-trim)),pr*sc,'linewidth',3);
xlim([0 D_max]);
ylim([0 r_max]);
...
set(gcf,'color','white');
zticklabels([]);
ylabel('$R~ (s^{-1})$','interpreter','latex','Rotation',20);
