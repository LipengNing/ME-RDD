function ShowDDF2_SimTrue(p,D,r,D_max,r_max)
if(~exist('D_max','var'))
    D_max=2;
end
...
Nd = length(D);
Nr = length(r);
imagesc(D,r,ppp,[0 0.05]);
hold on;
LL = watershed(-ppp);

id = find(LL==0);
for i = 1:length(id)
    [x,y] = ind2sub(size(ppp),id(i));
    plot3(D(y),r(x),1e-4,'r.','markersize',4);
end
...
hold on;plot3(zeros(1,Nr),r,pr,'linewidth',3);
plot3(D,zeros(1,Nd),pD,'linewidth',3);
ylim([0 r_max]);
xlim([0 D_max]);
